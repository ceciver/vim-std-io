...
let g:std_io_run_commands = {'cpp': "'g++ -DLOCAL -Wall -Wextra -O2 -g -fsanitize=address -fsanitize=undefined -D_GLIBCXX_DEBUG --std=c++17 -o ' . expand('%:p:r') . '.o ' . expand('%:p') . ' && ' . expand('%:p:r') . '.o'"}", 'java': "'javac ' . expand('%:p') . ' && java ' . expand('%:r')", 'python': "'python' . expand('%:p')" }
